...
class Inventario_Model extends Model
{
    ...
    public function ajuste_insumo_buscar($data)
    {
        try
        {
            ...
            //$sql = "SELECT * FROM v_productos WHERE (pro_nom LIKE :cadena OR pro_pre LIKE :cadena) AND crt_stock = 1";
            $sql = "SELECT * FROM v_insprod WHERE (ins_cod LIKE :cadena OR ins_nom LIKE :cadena) AND crt_stock = 1 ORDER BY ins_nom LIMIT 5";
            $result = $this->db->selectAll($sql, ["cadena" => $cadena], PDO::PARAM_STR);
