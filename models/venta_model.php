    ...
    */
    ...
    public function listarProdsMasVend($data)
    {
        ...
        {   
            ...
            $stm = $this->db->prepare("SELECT p.id_areap,p.id_pres,p.pro_nom,p.pro_pre".$campo.",p.pro_img 
                FROM tm_detalle_venta AS dv 
                INNER JOIN v_productos AS p ON dv.id_prod = p.id_pres 
                WHERE p.est_a = 'a' AND p.est_b = 'a' AND p.est_c = 'a' ".$variable);
            $stm->execute();
            $c = $stm->fetchAll(PDO::FETCH_OBJ);
            foreach($c as $k => $d)
            {
    ...
    public function RegistrarPedido($data)
{
    ...
    public function ListarDetallePed($data)
    {
        ...
        {   
            ...
            $stm->execute(array($data['id_pedido']));
            $c = $stm->fetch(PDO::FETCH_OBJ);
            $config = $this->db->SelectOne("SELECT name_pdf FROM tm_configuracion WHERE id_cfg = 1");
            $c->{'NamePdf'} = $config;
            /* Traemos el detalle */
    ...
    */
    ...
    public function getVenta($data, $usuarioId, $apcId, $igv, $fecha) {
        // Consulta
        $consulta = "
            CALL usp_restEmitirVenta(
                :flag, :dividir_cuenta, :id_pedido, :tipo_pedido, :tipo_entrega, :id_cliente, 
                :id_tipo_doc, :id_tipo_pago, :id_usu, :id_apc, :pago_efe_none, :pago_tar, 
                :descuento_tipo, :descuento_personal, :descuento_monto, :descuento_motivo, 
                :comision_tarjeta, :comision_delivery, :igv, :total, :codigo_operacion, :fecha_venta
            );
        ";
        ...
        $arrayParam = [
            ...
            ':descuento_personal' => !empty($data['descuento_personal']) ? $data['descuento_personal'] : 0,
    ...
    public function getApiVenta($url, $token, $datos) {
        $curl = curl_init();
        $headers = [
        ...
        // Aseguramos que la respuesta sea decodificada correctamente
        $response = json_decode($response, true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            return [
                'error' => true,
                'message' => 'Error al decodificar la respuesta del API: ' . json_last_error_msg()
            ];
        }
        ...
        if (!isset($response['success'])) {
            return [
                'error' => true,
                'message' => 'Respuesta inesperada del API.'
            ];
        }
    
        if ($response['success'] === true) {
            $response_send = [
                'code_respuesta_sunat' => $response['data']['state_type_id'] ?? null,
                'name_file_sunat' => $response['data']['filename'] ?? null,
                'hash_cpe' => $response['data']['hash'] ?? null,
                'external_id' => $response['data']['external_id'] ?? null
            ];
    
            if (!empty($response['data']['qr'])) {
                $imagenBinaria = base64_decode($response['data']['qr']);
                $nombreArchivo = ($response['data']['filename'] ?? uniqid()) . ".png";
            ...
            return [
                'error' => true,
                'message' => $response['message'] ?? 'Ocurrió un error desconocido en el API.'
            ];
        }
    }
    ...
    //             //datos a usar
    //             $fecha = date("Y-m-d H:i:s");
    //             $orden_compra = $this->generarCodigo(10);
            
    //             //datos de session
    //             $api_url = Session::get('api_url_pro');
    //             $api_token = Session::get('api_token_pro');
    //             $usuarioId = Session::get('usuid');
    //             $apcId = Session::get('apcid');
    //             $igv = Session::get('igv');
    //             $modoApi = Session::get('f_pro');
    //             $tipoOperation = Session::get('type_operation');
    //             //$idventa = $this->getVenta($data, $usuarioId, $apcId, $igv, $fecha);
    ...
    //             if (!is_null($cliente_prepare) && $cliente_prepare !== "") {
    //                 $cliente_resultado = explode(" | ", $cliente_prepare);
    //             } else {
    //                 $cliente_resultado = [ 0 => '00000000', 1 => 'Cliente Varios', 2 => '-', 3 => '-'];
    //             }
    //             //$cliente_resultado = explode(" | ", $cliente_prepare);
                
    //             if (count($cliente_resultado) >= 4) {
    //                 $datos_cliente = [
    ...
    //             $totales = array(
    //                 "total_operaciones_gravadas" => ($tipoOperation == 2) ? 0.00 : $total_subtotal,
    //                 "total_operaciones_exoneradas" => ($tipoOperation == 2) ? $data['total'] : 0.00,
    //                 "total_igv" => ($tipoOperation == 2) ? 0.00 : $total_impuesto,
    //                 "total_impuestos" => ($tipoOperation == 2) ? 0.00 : $total_impuesto,
    //                 "total_valor" => ($tipoOperation == 2) ? $data['total'] : $total_subtotal,
    ...
    //             $b = $data['cantProd'];
    //             $c = $data['precProd'];
	// 			$d = isset($data['nameProd']) ? $data['nameProd'] : '';
    ...
    //                     //$this->db->prepare($sql)->execute(array($idventa, $a[$x], $b[$x], $c[$x]));
    //                     //guardando en el array para luego insertar
    //                     $dataToInsert[] = [
    //                         'id_venta' => 0,
    //                         'id_prod' => $a[$x],
    //                         'cantidad' => $b[$x],
    ...
    //                     //20 gratuito 10 gravada
    //                     $item = array(
    //                         "codigo_interno" => $a[$x],
    //                         "descripcion" => $d[$x],
    //                         "unidad_de_medida" => "NIU",
    //                         "cantidad" => $b[$x],
    //                         "valor_unitario" => ($tipoOperation == 2) ? $c[$x] : $valor_unitario,
    //                         "codigo_tipo_precio" => "01",
    //                         "precio_unitario" => $c[$x],
    //                         "codigo_tipo_afectacion_igv" => ($tipoOperation == 2) ? "20" : "10",
    //                         "total_base_igv" => ($tipoOperation == 2) ? $total_c_igv : $total_no_igv,
    //                         "porcentaje_igv" => $igv,
    //                         "total_igv" => ($tipoOperation == 2) ? 0.00 : $impuesto,
    //                         "total_impuestos" => ($tipoOperation == 2) ? 0.00 : $impuesto,
    //                         "total_valor_item" => ($tipoOperation == 2) ? $total_c_igv : $total_no_igv,
    ...
    //             if ($modoApi == 3 && in_array($data['tipo_doc'], [1, 2])) {
    //                 $sendApi = $this->getApiVenta($api_url, $api_token, $versiones);

    //                 //manejar el api
    //                 if (isset($sendApi['error']) && $sendApi['error'] === true) {
    //                     // Manejar el error de la API
    //                     return ['success' => false, 'message' => $sendApi['message']];
    //                 } else {
    //                     // Verificar si los datos esperados están presentes en la respuesta
    //                     if (isset($sendApi['external_id'])) {

    //                         $idventa = $this->getVenta($data, $usuarioId, $apcId, $igv, $fecha);

    //                         foreach ($dataToInsert as $datos) {
    ...
    //                         $this->db->update('tm_venta', $sendApi, ['id_venta' => $idventa]);
    ...
    //         } else {
    //             return ['success' => false, 'message' => 'Se necesitan productos para el comprobante.'];
    //         }
    //     } catch (Exception $e) {
    ...
    public function RegistrarVenta($data) {
        try{
            ...
            if( ($conteo->conteo > 0) &&  $data['total'] > 0){
                ...
                $modoApi = Session::get('f_pro');
                $tipoOperation = Session::get('type_operation');
                $tipoOperation = 1; // Forzar a "Exonerada - Operación Onerosa"
                ...
                $totales = array(
                    ...
                    "total_operaciones_exoneradas" => $data['total'],
                    "total_igv" => 0.00,
                    "total_impuestos" => 0.00,
                    "total_valor" => $data['total'],
                    "total_venta" => $data['total']
                );
                if($data['descuento_monto'] > 0) {
                    $totales["total_descuentos"] = number_format($data['descuento_monto'],2);
                ...
                for ($x = 0; $x < $count; ++$x) {
                    if ($b[$x] > 0) {
                        ...
                        $item = array(
                            ...
                            "codigo_tipo_precio" => "01",
                            "precio_unitario" => $c[$x],
                            "codigo_tipo_afectacion_igv" => "20", // Exonerado
                            "total_base_igv" => $total_c_igv,
                            "porcentaje_igv" => 0.00,
                            "total_igv" => 0.00,
                            "total_impuestos" => 0.00,
                            "total_valor_item" => $total_c_igv,
                            "total_item" => $total_c_igv
                        );
    ...
    public function contadorPedidosPreparados()
    {
        try {
            ...
            $sql = "SELECT COUNT(id_pedido) AS cantidad FROM v_cocina_me WHERE id_tipo = 0 AND cantidad > 0 AND estado = 'c'";

            if ($rol == 0 || $rol == 1 || $rol == 2 || $rol == 3) {
    ...
    public function listarPedidosPreparados()
    {
        ...
        {      
            if(Session::get('rol') == 1 OR Session::get('rol') == 2 OR Session::get('rol') == 3){
                $stm = $this->db->prepare("SELECT * FROM v_cocina_me WHERE id_tipo = 0 AND cantidad > 0 AND estado = 'c'");
                $stm->execute();   
            } elseif(Session::get('rol') == 5){
                $stm = $this->db->prepare("SELECT * FROM v_cocina_me WHERE id_tipo = 0 AND id_mozo = ? AND cantidad > 0 AND estado = 'c'");
