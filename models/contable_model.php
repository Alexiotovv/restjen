        ...
        try
        {
            ...
            foreach($c as $k => $d)
            {
                ...
                $c[$k]->{'Detalle'} = $this->db->query("SELECT v_productos.pro_cod AS codigo_producto, 
                    ...
                    IF(v_productos.pro_imp='1',ROUND((tm_detalle_venta.precio/(1 + ".$this->igv_universal.")),2),tm_detalle_venta.precio) AS valor_unitario,
                    tm_detalle_venta.precio AS precio_unitario,
                    IF(v_productos.pro_imp='1',ROUND((tm_detalle_venta.precio/(1 + ".$this->igv_universal."))*tm_detalle_venta.cantidad,2),
                    ROUND(tm_detalle_venta.precio*tm_detalle_venta.cantidad,2)) AS valor_venta,
                    IF(v_productos.pro_imp='1',ROUND((tm_detalle_venta.precio/(1 + ".$this->igv_universal.")*tm_detalle_venta.cantidad)*".$this->igv_universal.",2),0) AS total_igv 
