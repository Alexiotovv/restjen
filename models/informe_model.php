...
class Informe_Model extends Model
{
    ...
    public function venta_all_imp_($data)
    {
        try
        {
            ...
            $detalle_stm = $this->db->prepare("
                SELECT 
                    v_productos.pro_cod AS codigo_producto, 
                    IF(v_productos.pro_imp='1','10','20') AS codigo_afectacion, 
                    CAST(tm_detalle_venta.cantidad AS DECIMAL(7,2)) AS cantidad, 
                    IF(v_productos.pro_imp='1',ROUND((tm_detalle_venta.precio/(1 + ?)),2),tm_detalle_venta.precio) AS valor_unitario, 
                    tm_detalle_venta.precio AS precio_unitario, 
                    IF(v_productos.pro_imp='1',ROUND((tm_detalle_venta.precio/(1 + ?))*tm_detalle_venta.cantidad,2),ROUND(tm_detalle_venta.precio*tm_detalle_venta.cantidad,2)) AS valor_venta, 
                    IF(v_productos.pro_imp='1',ROUND((tm_detalle_venta.precio/(1 + ?)*tm_detalle_venta.cantidad)*?,2),0) AS total_igv,
                    CASE 
                        WHEN tm_configuracion.name_pdf = 2 THEN CONCAT(v_productos.pro_nom, ' - ', v_productos.pro_pre)
                        WHEN tm_configuracion.name_pdf = 0 THEN v_productos.pro_nom
                        WHEN tm_configuracion.name_pdf = 1 THEN v_productos.pro_pre
                    END AS nombre_producto
                FROM 
                    tm_detalle_venta 
                INNER JOIN 
                    tm_venta ON tm_detalle_venta.id_venta = tm_venta.id_venta 
                INNER JOIN 
                    v_productos ON tm_detalle_venta.id_prod = v_productos.id_pres 
                INNER JOIN 
                    tm_configuracion ON tm_configuracion.id_cfg = 1 
                WHERE 
                    tm_venta.id_tipo_doc IN ('1','2','3') 
                    AND tm_detalle_venta.precio > 0 
                    AND tm_detalle_venta.id_venta = ?");
