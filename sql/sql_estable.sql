...
CREATE TABLE `tm_configuracion` (
  ...
  `name_pdf` int(1) NOT NULL DEFAULT 1,
  PRIMARY KEY (`id_cfg`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `tm_configuracion` VALUES (1,'America/Lima','RUC',11,'DNI',8,'IGV',10.00,'SOL','S/','PES-101ISL03','192.168.18.51',0,0,0,0,0,1,0,'123456','{"body":"dark-mode layout-navbar-fixed","nav-sidebar":"","main-header":"bg-dark","main-sidebar":"sidebar-dark-primary","brand-link":""}', 1, 1);
...
INSERT INTO `tm_empresa` VALUES (1,'20610799923','LAS BRASAS DE VILLA S.A.C.','LAS BRASAS DE VILLA S.A.C.','AV TALARA MZ P LT 3 SECTOR 3 GRUPO 24 VILLA EL SALVADOR','AV TALARA MZ P LT 3 SECTOR 3 GRUPO 24 VILLA EL SALVADOR','150132','LIMA','LIMA','VILLA EL SALVADOR',1,'https://demo.facturadorpro.com','aOSQNnmXpnf3lcRtfRaHadEmkK8EIHJtsvKgTKxLHCEWmqTbsC',3,'MODDATOS','MODDATOS','institutoisi','052636a1-befe-4591-8c83-aadf0e2854a7','Us23ZfDVYxNpCqx75wlztg==','logoprint.png','961403072','','');
...
CREATE TABLE `tm_mesa` (
  ...
  `forma` int(1) NOT NULL DEFAULT 0,
  PRIMARY KEY (`id_mesa`),
...
/*!50003 SET collation_connection  = utf8mb4_general_ci */ ;
DELIMITER ;;
CREATE DEFINER=CURRENT_USER PROCEDURE `usp_configMesas`(IN `_flag` INT(11), IN `_id_mesa` INT(11), IN `_id_salon` INT(11), IN `_nro_mesa` VARCHAR(5), IN `_estado` VARCHAR(45), IN `_forma` INT(1))
BEGIN
	DECLARE _filtro INT DEFAULT 0;
	DECLARE _cod0 INT DEFAULT 0;
	DECLARE	_cod1 INT DEFAULT 1;
	...
	IF _flag = 1 THEN
		...
		IF _filtro = 0 THEN
			INSERT INTO tm_mesa (id_salon,nro_mesa,estado,forma) VALUES (_id_salon, _nro_mesa, _estado, _forma);
			SELECT _cod1 AS cod;
		ELSE
			SELECT _cod0 AS cod;
		END IF;
	...
	IF _flag = 2 THEN
		...
		IF _filtro = 0 THEN
			UPDATE tm_mesa SET nro_mesa = _nro_mesa, estado = _estado, forma = _forma WHERE id_mesa = _id_mesa;
			SELECT _cod2 AS cod;
		ELSE
			SELECT _cod2 AS cod;
		END IF;
...
BEGIN
    ...
    IF _flag = 1 THEN
        ...
        WHERE presentacion = _presentacion 
        AND id_prod = _id_prod;
        
        IF _cont = 0 THEN
            INSERT INTO tm_producto_pres (
                id_prod, cod_prod, presentacion, descripcion, precio, precio_delivery, receta, stock_min, crt_stock, impuesto, delivery, margen, igv, imagen, estado
            ) 
            VALUES (
                _id_prod, _cod_prod, _presentacion, _descripcion, _precio, _precio_delivery, _receta, _stock_min, _stock_limit, _impuesto, _delivery, _margen, _igv, _imagen, _estado
            );
            SELECT _cod1 AS cod;
        ELSE
            SELECT _cod0 AS cod;
    ...
    IF _flag = 2 THEN
        ...
        SET cod_prod = _cod_prod, presentacion = _presentacion, descripcion = _descripcion, precio = _precio, precio_delivery = _precio_delivery, receta = _receta, stock_min = _stock_min, crt_stock = _stock_limit, impuesto = _impuesto, delivery = _delivery, margen = _margen, igv = _igv, imagen = _imagen, estado = _estado 
...
BEGIN
	...
	IF _flag = 1 THEN
		SELECT 
			...
			LPAD(
				IFNULL(
					MAX(CAST(SUBSTRING(v.nro_doc, LENGTH(v.nro_doc) - 7, 8) AS UNSIGNED)), 
					td.numero
				) + 1, 
				8, 
				'0'
		...
		LEFT JOIN 
			tm_venta AS v 
		ON 
			v.id_tipo_doc = td.id_tipo_doc AND v.serie_doc = td.serie
		WHERE 
			td.id_tipo_doc = _id_tipo_doc 
		GROUP BY 
			td.serie;

		INSERT INTO tm_venta (
			id_pedido, id_tipo_pedido, id_cliente, id_tipo_doc, id_tipo_pago, id_usu, id_apc, serie_doc, nro_doc, pago_efe, pago_efe_none, pago_tar, descuento_tipo, descuento_personal, descuento_monto, descuento_motivo, comision_tarjeta, comision_delivery, igv, total, codigo_operacion, fecha_venta
		)
		VALUES (
			_id_pedido, _tipo_pedido, _id_cliente, _id_tipo_doc, _id_tipo_pago, _id_usu, _id_apc, @serie, @numero, pago_efe, _pago_efe_none, pago_tar, _descuento_tipo, _descuento_personal, _descuento_monto, _descuento_motivo, _comision_tarjeta, _comision_delivery, _igv, _total, _codigo_operacion, _fecha_venta
		);
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=CURRENT_USER SQL SECURITY DEFINER */
/*!50001 VIEW `v_listar_mesas` AS select `vm`.`id_mesa` AS `id_mesa`,`vm`.`id_salon` AS `id_salon`,`vm`.`nro_mesa` AS `nro_mesa`,`vm`.`estado` AS `estado`,`vm`.`desc_salon` AS `desc_salon`,`vo`.`id_pedido` AS `id_pedido`,`vo`.`fecha_pedido` AS `fecha_pedido`,`vo`.`nro_personas` AS `nro_personas`,`vm`.`forma` AS `forma` from (`v_mesas` `vm` left join `v_pedido_mesa` `vo` on(`vm`.`id_mesa` = `vo`.`id_mesa`)) order by `vm`.`nro_mesa` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=CURRENT_USER SQL SECURITY DEFINER */
/*!50001 VIEW `v_mesas` AS select `m`.`id_mesa` AS `id_mesa`,`m`.`id_salon` AS `id_salon`,`m`.`nro_mesa` AS `nro_mesa`,`m`.`estado` AS `estado`,`cm`.`descripcion` AS `desc_salon`,`m`.`forma` AS `forma` from (`tm_mesa` `m` join `tm_salon` `cm` on(`m`.`id_salon` = `cm`.`id_salon`)) where `m`.`id_mesa` <> 0 and `cm`.`estado` <> 'i' order by `m`.`id_mesa` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 VIEW `v_pedido_mesa` AS SELECT 
    ...
    vm.nro_mesa,
    vm.desc_salon,
    vm.estado AS estado_mesa,
    CONCAT(u.nombres, ' ', u.ape_paterno) AS nombre_mozo
FROM 
    tm_pedido p
JOIN 
    tm_pedido_mesa pm ON p.id_pedido = pm.id_pedido
JOIN 
    v_mesas vm ON pm.id_mesa = vm.id_mesa
JOIN 
    tm_usuario u ON pm.id_mozo = u.id_usu
JOIN 
    (SELECT id_mesa, MAX(id_pedido) AS max_pedido
     FROM tm_pedido_mesa
     GROUP BY id_mesa) AS latest ON pm.id_mesa = latest.id_mesa AND p.id_pedido = latest.max_pedido
WHERE 
    p.estado = 'a'
ORDER BY 
    p.id_pedido DESC */;
