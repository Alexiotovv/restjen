...
$(function() {
    ...
    .on('success.form.fv', function(e) {
        ...
        /*for (var pair of presentacion.entries()) {
    ...
    .on('success.form.fv', function(e) {
        ...
        if (e.type == "submit") {
            ...
            $.ajax({
                ...
                success: function (response) {
                    let titulo, icono, htmls = null, htmlErrors = '', htmlSalida = null, text = null;
            
                    if (response.success == 1) {
                        // En caso de éxito, usar HTML
                        titulo = 'Proceso Terminado';
                        icono = 'success';
                        htmls = `
                            <strong><i class="fas fa-bell mr-1 fa-lg text-info"></i> Mensajes del sistema</strong><br>
                            <span class="text-muted">${response.prod}</span><br>
                            <span class="text-muted">${response.pres}</span><br>
                            <div id="messageErrorsImportResult" style="display: none;">
                            </div>
                        `;
                        Swal.fire({
                            title: titulo,
                            html: htmls, // Esto será nulo si no es éxito
                            text: text,  // Esto será nulo si es éxito
                            icon: icono,
                            confirmButtonColor: "#34d16e",
                            confirmButtonText: "Aceptar",
                            allowOutsideClick: false,
                        ...
                        // Verificar y construir HTML para errores
                        htmlErrors = `<strong><i class="fas fa-exclamation-circle mr-1 fa-lg text-danger"></i> Errores en la importación</strong><br>`;
      
                        if (response.proderror.prodExist && response.proderror.prodExist.length !== 0) {
                            htmlErrors += `<span class="text-muted">${response.proderror.prodExist}</span><br>`;
                            $('#messageErrorsImportResult').show()
                        }
            
                        if (response.proderror.areaPnotExist && response.proderror.areaPnotExist.length !== 0) {
                            htmlErrors += `<span class="text-muted">${response.proderror.areaPnotExist}</span><br>`;
                            $('#messageErrorsImportResult').show()
                        }
            
                        if (response.proderror.prodNotExist && response.proderror.prodNotExist.length !== 0) {
                            htmlErrors += `<span class="text-muted">${response.proderror.prodNotExist}</span><br>`;
                            $('#messageErrorsImportResult').show()
                        }
                            
                        if(response.preserror.presExist.length !== 0) {
                            htmlErrors += `<span class="text-muted">${response.preserror.presExist}</span><br>`;
                            $('#messageErrorsImportResult').show();
                        }
                              
                        if(response.preserror.codProdExist.length !== 0) {
                            htmlErrors += `<span class="text-muted">${response.preserror.codProdExist}</span><br>`;
                            $('#messageErrorsImportResult').show();
                        }
                    
                        if(response.preserror.presNotExist.length !== 0) {
                            htmlErrors += `<span class="text-muted">${response.preserror.presNotExist}</span><br>`;
                            $('#messageErrorsImportResult').show();
                        }

                        $('#messageErrorsImportResult').html(htmlErrors);
                            
                    } else {
                        // En caso contrario, usar texto simple
                        Swal.fire({
                            title: 'Proceso No Culminado',
                            text: response.message,  // Esto será nulo si es éxito
                            icon: 'error',
                            confirmButtonColor: "#34d16e",
                            confirmButtonText: "Aceptar",
                            allowOutsideClick: false,
            ...
            /*
                    Swal.fire({
                        title: titulo,
                        html: htmlSalida, // Esto será nulo si no es éxito
                        text: text,  // Esto será nulo si es éxito
                        icon: icono,
                        confirmButtonColor: "#34d16e",
                        confirmButtonText: "Aceptar",
...
$('#presentacion_presentacion').keyup(function(){
    $("#cod_prod_presentacion").val(generarCodigo(9));
});
