...
/*estilos*/
$directorio = 'public/css/colors';
$files = scandir($directorio);
$cssFiles = array_filter($files, function($file) use ($directorio) {
    return is_file($directorio . '/' . $file) && pathinfo($file, PATHINFO_EXTENSION) === 'css';
});
?>
                ...
                <div class="row floating-labels m-t-40">
                    ...
                    <div class="form-group col-md-4">
                        <label for="estilos">Estilos</label>
                            <select class="form-control" id="estilos" name="estilos">
                                <!--<option selected disabled>Turnos disponibles en caja...</option>-->
                                <?php
                                foreach ($cssFiles as $file) {
                                    $nameStyle = pathinfo($file, PATHINFO_FILENAME);
                                    if($nameStyle === STYLE_CSS)
                                    {
                                        echo '<option selected value="'.$nameStyle.'">' . $nameStyle . '</option>';
                                    } else {
                                        echo '<option value="'.$nameStyle.'">' . $nameStyle . '</option>';
                                    }
                                }
                                ?>
                            </select>
