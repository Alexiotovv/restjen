...
var listarProductos = function(id_catg,side = false){
    ...
    $.ajax({
        ...
        error: function (xhr, status, error) {
            ...
            $('#list-productos').html('<div class="text-center"><h4 class="ich m-t-none" style="color: red;"><br><br><br><br><br><br><br><br><br><i class="mdi mdi-alert-circle display-4"></i><br>Ocurrió un error al cargar los productos. Inténtalo más tarde.</h4></div>');
        }
...
var facturar = function(id_pedido,dividir_cuenta){
    ...
    $.ajax({
        ...
        success: function (data) {
            //var count_for = 0;
            let NamePdf = data.NamePdf.name_pdf, count_for = 0, NameComplete = '';
            
            $.each(data.Detalle, function(i, item) {
                if(NamePdf === 0) {
                    NameComplete = item.Producto.pro_nom;
                } else if(NamePdf === 1) {
                    NameComplete = item.Producto.pro_pre
                } else {
                    NameComplete = item.Producto.pro_nom + ' - ' + item.Producto.pro_pre;
                }
                ...
                if (1 == dividir_cuenta && item.cantidad > 0){
                    ...
                    $('<tr class="comment-list"/>')
                        ...
                        .append(
                            $('<td width="60%"/>')
                            .html('<input type="hidden" name="idProd[]" value="'+item.id_pres+'"/><input type="hidden" name="nameProd[]" value="'+NameComplete+'"/>'
                                + '<span class="name_pro_fac">' + item.Producto.pro_nom + ' </span><span class="label label-warning text-uppercase pre_pro_fac">'+item.Producto.pro_pre+' - '+NameComplete+'</span>')
                            )
