    ...
    $.ajax({
        ...
        success: function (item) {
            ...
            $.each(item['mesa'], function(i, mesa) {
                ...
                forma = (mesa.forma == 0) ? '' : 'rounded-circle';
                ...
                if($('#rol_usr').val() == 5){
                    ...
                    //mesa en proceso de pago
                        ...
                        $('.list-mesas')
                            $('.list-mesas')
                            .append($('<input type="hidden" name="tiempo_mesa[]" value="'+mesa.fecha_pedido+'"/>'
                                +'<button class="btn btn-blue '+forma+' dim btn-large-dim" onclick="listarPedidos('+mesa.id_salon+','+mesa.id_mesa+',1,'+mesa.id_pedido+',\''+mesa.nro_mesa+'\',\''+mesa.desc_salon+'\')">'
                                +'<span class="span-a"><i class="far fa-user"></i> '+mesa.nro_personas+'</span><span>'+mesa.nro_mesa+'</span><br><img src="public/images/other_icon/full.png" width="40px" height="40px"/>'
                        ...
                        $('.list-mesas')
                            .append($('<input type="hidden" name="tiempo_mesa[]" value="'+mesa.fecha_pedido+'"/>'
                                +'<button class="btn btn-reed '+forma+' dim btn-large-dim" onclick="listarPedidos('+mesa.id_salon+','+mesa.id_mesa+',1,'+mesa.id_pedido+',\''+mesa.nro_mesa+'\',\''+mesa.desc_salon+'\')">'
...
var listarPedidos = function(cod_salon,cod_mesa,id_tipo_pedido,id_pedido,mesa,salon){
    ...
    $.ajax({
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            ...
            $('#list-pedidos').html('<div class="justify-center" style="height: 100%;"><div class="text-center"><h4 style="color: red;">Ocurrió un error al cargar los pedidos.</h4><p>' + errorThrown + '</p></div></div>');
        }
